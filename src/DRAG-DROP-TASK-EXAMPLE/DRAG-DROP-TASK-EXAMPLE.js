export class Task {
    ...
    Если кратко, то нужно подключить библиотку tinify (maven указан в комментарии), создать файл, аналогичный
    любой другой задаче, см. примеры в файлах AT_01.java и т.п. Кода пишется очень мало, всё объединяется само,
    надо только правильно назвать файлы с картинками.
    Для объединения берутся файлы из каталога с задачей. Они должны начинаться с подчеркивания, а если это
    svg файлы, надо написать размер результата в пикселях, например, _a_20_30.svg, т.е. 20 на 30 пикселей.
    (Еще можно дописать обрезание картинки с помощью _mcr10x10, но это нужно очень редко)
